...
class Assistant(object):
    ...
    def __init__(self):
        ...
        self.send_mail = global_config.getboolean('mail', 'enable')
        self.message = message(global_config.get('mail', 'mail_addr')) if self.send_mail else None
        self.item_cat = dict()
    ...
    def make_reserve_on_time(self, sku_id, buy_time):
        t = Timer(buy_time=buy_time)
        t.start()
        tp = 0
        while True:
            result = self.make_reserve(sku_id)
            tp += 1
            if result==1 or tp==100:
                break
    ...
    def make_reserve(self, sku_id):
        ...
        try:
            ...
            if '未开始' in reserve_result:
                logger.info('%s 尚未开始预约', sku_id)
                return 0
            elif '预约过' in reserve_result:
                logger.info('%s 已经预约过', sku_id)
                return 1
            elif '成功' in reserve_result:
                logger.info('%s 预约成功', sku_id)
                return 1
            else:
                logger.info('%s 其他原因导致预约失败: %s',sku_id, reserve_result)
                return 2
        except:
            return 2
    ...
    def submit_order(self):
        ...
        try:
            ...
            if resp_json.get('success'):
                ...
                if self.send_mail:
                    msg = str(order_id) + ' 已经下单了。24小时内付款'
                    self.message.send(msg, True)
                    # self.messenger.send(text='jd-assistant 订单提交成功', desp='订单号：%s' % order_id)
                return True
            else:
                messag, result_code = resp_json.get('message'), resp_json.get('resultCode')
                if result_code == 0:
                    self._save_invoice()
                    messag = messag + '(下单商品可能为第三方商品，将切换为普通发票进行尝试)'
                elif result_code == 60077:
                    messag = messag + '(可能是购物车为空 或 未勾选购物车中商品)'
                elif result_code == 60123:
                    messag = messag + '(需要在config.ini文件中配置支付密码)'
                logger.info('订单提交失败, 错误码：%s, 返回信息：%s', result_code, messag)
                logger.info(resp_json)
                return False
        except Exception as e:
            logger.error(e)
    ...
    def _get_seckill_url(self, sku_id):
        ...
        for _ in range(10):
            ...
            if resp_json.get('url'):
                ...
                logger.info("抢购链接获取失败，%s不是抢购商品或抢购页面暂未刷新，立即重试，state: %s", sku_id, resp_json.get('state'))
                # time.sleep(0.3)
        logger.info("抢购链接获取失败，%s 不是抢购商品或抢购页面暂未刷新", sku_id)
        return None
    ...
    def request_seckill_url(self, sku_id):
        ...
        if not self.seckill_url.get(sku_id):
            ...
            if self.seckill_url[sku_id] is None:
                return False;
    ...
    def submit_seckill_order(self, sku_id, num=1):
        ...
        resp_json = ''
        if 'marathon.jd.com/koFail.html' in resp.text:
            logger.info('尚未开始抢购')
            return False, 0
        else:
            try:
        ...
        if resp_json.get('success'):
            ...
            time_l = 0
            if resp_json.get('resultCode')==60017:
                time_l = 0.1
            logger.info('抢购失败，返回信息: %s', resp_json)
            return False, time_l

    def exec_seckill(self, sku_id, retry=10, interval=0, num=1):
        """立即抢购
        ...
        for count in range(1, retry + 1):
            ...
            if not self.request_seckill_url(sku_id):
                break
            # self.request_seckill_checkout_page(sku_id, num)
            succ, tim = self.submit_seckill_order(sku_id, num)
            if succ:
                return True
            else:
                interval += tim
    ...
    def exec_seckill_by_time(self, sku_ids, buy_time, retry=10, interval=0.5, num=1):
        """定时抢购
        ...
        for sku_id in items_dict:
            ...
            if self.exec_seckill(sku_id, retry, interval, num):
                logger.info('%s抢购成功!', sku_id)
