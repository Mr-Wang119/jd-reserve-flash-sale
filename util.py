...
def parse_time(time_s):
    ...
    将时间解析为list
    '''
    if isinstance(time_s, list):  # 防止重复解析
        return time_s
    time_list = list(filter(bool, map(lambda x: x.strip(), time_s.split(','))))
    result = []
    local_time = time.strftime("%Y-%m-%d ",time.localtime(time.time()))
    for item in time_list:
        result.append(local_time+item)
    return result

def parse_sku_id_to_list(sku_ids):
